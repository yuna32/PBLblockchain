...
const CONTRACT_MAP = {
  ...
  PonziLabPatched: { type: "ponzi_patched",  csv: "evasion_patched_log.csv"}
};

// Fraud type → ontology class mapping
const TYPE_TO_ONTOLOGY_CLASS = {
  ponzi:        "PonziScheme",
  rugpull:      "RugPull",
  laundering:   "MoneyLaundering",
  pumpdump:     "PumpAndDump",
  honeypot:     "HoneypotTrap",
  ponzi_patched:"PonziScheme",
  normal:       null
};

// Build ontology_mapping section for the report
function buildOntologyMapping(type, staticResult, dynamicResult) {
  const fraudClass = TYPE_TO_ONTOLOGY_CLASS[type] || null;
  if (!fraudClass || !FraudOntology.fraudTypes[fraudClass]) return null;

  const ft = FraudOntology.fraudTypes[fraudClass];

  // Determine evasion techniques from static + dynamic triggered rules
  const staticIds  = staticResult.triggered_rules.map(r => r.id);
  const dynamicIds = (dynamicResult.triggered_rules || []).map(r => r.id);
  const evasionDetected = [];
  if (staticIds.includes("UNCONSTRAINED_OWNER_DRAIN") || staticIds.includes("RAPID_EXIT_MECHANISM"))
    evasionDetected.push("OwnerWithdrawAll");
  if (staticIds.includes("HIDDEN_WITHDRAW_BLOCK"))
    evasionDetected.push("HiddenRequireCondition");
  if (dynamicIds.includes("CONCENTRATION_DRAIN"))
    evasionDetected.push("SmurfingDeposit");
  if (dynamicIds.includes("ZERO_WITHDRAW_PATTERN") && !evasionDetected.includes("HiddenRequireCondition"))
    evasionDetected.push("HiddenRequireCondition");

  // Check which necessary conditions were met
  const conditionRuleMap = {
    BalanceDrop:       "BALANCE_DROP",
    FlowSpike:         "FLOW_SPIKE",
    ConcentrationDrain:"CONCENTRATION_DRAIN",
    ZeroWithdrawBlock: "ZERO_WITHDRAW_PATTERN"
  };
  const necessaryMet = ft.necessaryConditions.filter(c => {
    const rId = conditionRuleMap[c];
    return rId && dynamicIds.includes(rId);
  });

  // Collect recommended patches from evasion techniques
  const recommendedPatches = [...new Set(evasionDetected)]
    .map(et => FraudOntology.evasionTechniques[et]?.patch)
    .filter(Boolean);

  return {
    fraud_class:                fraudClass,
    parent_class:               "FraudPattern",
    label:                      ft.label,
    necessary_conditions_met:   necessaryMet,
    evasion_techniques_detected:evasionDetected,
    affected_roles:             ft.affectedRoles,
    recommended_patches:        recommendedPatches,
    gui_emphasis:               ft.guiEmphasis || null
  };
}
...
async function run() {
  ...
  // ── Step 0: 예방 추론 (Prevention Reasoning) ──────────────────────────────
  printSection("Step 0 / 4  온톨로지 예방 추론 (Prevention Reasoner)");
  const preventionResult = await runPrevention(contractName);
  if (preventionResult.error) {
    console.log("  Warn: " + preventionResult.error);
  } else {
    const pl = preventionResult.risk_level;
    const plColor = { CRITICAL: C.red, HIGH: "\x1b[33m", MEDIUM: C.yellow, LOW: C.green }[pl] || C.gray;
    console.log("  " + C.gray + "의심 사기 유형:" + C.reset + " " + (preventionResult.fraud_type_suspected || "미검출"));
    console.log("  " + C.gray + "예방 위험 점수:" + C.reset + " " + plColor + preventionResult.risk_score + C.reset + "  [" + pl + "] — " + preventionResult.risk_label);
    const detected = preventionResult.checklist.filter(c => c.detected);
    console.log("  " + C.gray + "체크리스트 위험 항목:" + C.reset + " " + detected.length + " / " + preventionResult.checklist.length);
    if (detected.length) {
      console.log("  " + C.gray + "상위 2개 수정 제안:" + C.reset);
      for (const c of detected.slice(0, 2)) {
        console.log("    " + C.red + "X" + C.reset + " [" + c.id + "] " + c.fix);
      }
    }
    console.log("  " + C.gray + "온톨로지 추론 체인:" + C.reset);
    for (const step of preventionResult.ontology_reasoning_chain) {
      console.log("    " + C.gray + "-> " + step + C.reset);
    }
    console.log("  " + C.gray + "배포 권고:" + C.reset + " " + plColor + preventionResult.deployment_recommendation + C.reset);
  }
  ...
  if (dynamicResult.error) {
    ...
    if (dynamicResult.evasion_detected) {
      console.log(`  ${C.gray}── 회피 탐지 ──${C.reset}`);
      printKeyVal("회피 서브클래스", dynamicResult.evasion_subclass, C.yellow);
      printKeyVal("회피 결과      ", dynamicResult.evasion_consequence, C.yellow);
      printKeyVal("대응 탐지      ", dynamicResult.counter_detection, C.cyan);
    }
  }
  ...
  const ontologyMapping = buildOntologyMapping(type, staticResult, dynamicResult);
