...
FRAUD_CONFIGS = {
    ...
    "honeypot":   {"label": "허니팟",        "color": "#f39c12", "file": "honeypot_log.csv"},
}

ALPHA = 0.85
DEPOSIT_ACTIONS  = {"deposit", "stake", "fund_reward_pool"}
WITHDRAW_ACTIONS = {"withdraw", "unstake", "owner_withdraw_all", "owner_collect"}
...
for key, d in datasets.items():
    ...
    # Honeypot: mark withdraw_attempt blocks with red X on balance chart
    if key == "honeypot" and "action" in d["df"].columns:
        wa_blocks_df = d["df"][d["df"]["action"] == "withdraw_attempt"]
        wa_blocks = sorted(wa_blocks_df["block"].unique())
        for wb in wa_blocks:
            row_b = m[m["block"] == int(wb)]
            if not row_b.empty:
                bal_val = float(row_b["cumulative_balance"].iloc[0])
                ax.scatter([int(wb)], [bal_val], color="#e74c3c", marker="x",
                           s=120, linewidths=2.5, zorder=5,
                           label="출금 시도(실패)" if wb == wa_blocks[0] else "")
        if wa_blocks:
            ax.legend(fontsize=7)
